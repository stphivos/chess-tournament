    ...
    $.fn.gracket = function (method) {
            ...
            init: function (options) {
                ...
                for (var r = 0; r < round_count; r++) {
                    ...
                    for (var g = 0; g < game_count; g++) {
                        ...
                        var
                            game_html = helpers.build.game(this.gracket.settings, r + 1, g + 1),
                        ...
                        // append game
                        if (this.gracket.settings.scope && this.gracket.settings.compileService)
                            game_html = this.gracket.settings.compileService(game_html)(this.gracket.settings.scope);
        ...
        var helpers = {
            build: {
                ...
                game: function (node, round, no) {
                    return game = $("<div />", $.extend({}, node.gameAttrs, {
                        "class": node.gameClass,
                        "ng-click": "showGame(" + round + "," + no + ")"
                        //"round": round,
                        //"no": no
                    }));
                },
